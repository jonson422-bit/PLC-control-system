...
async def get_categories():
    ...
    with db.get_db() as conn:
        ...
        return {"categories": [row['category'] for row in cursor.fetchall()]}
...
async def upload_knowledge_file(file: UploadFile = File(...)):
    ...
    try:
        if filename.endswith('.json'):
            ...
            imported = import_json_knowledge(data)
        else:
            # 解析 Markdown 文件
            imported = import_markdown_knowledge(content.decode('utf-8'), filename)
