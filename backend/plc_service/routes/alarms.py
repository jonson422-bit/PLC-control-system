...
from ..database import (
    ...
    create_alarm_event, acknowledge_alarm, run_db
)
...
async def list_rules():
    """获取告警规则列表"""
    rules = await run_db(get_alarm_rules)
    for rule in rules:
        rule['condition'] = {
...
async def create_rule(rule: CreateAlarmRule):
    """创建告警规则"""
    rule_id = await run_db(lambda: create_alarm_rule(rule.model_dump()))
...
async def get_rule(rule_id: int):
    """获取单个告警规则"""
    rule = await run_db(lambda: get_alarm_rule_by_id(rule_id))
    if not rule:
...
async def update_rule(rule_id: int, rule: CreateAlarmRule):
    """更新告警规则"""
    existing = await run_db(lambda: get_alarm_rule_by_id(rule_id))
    if not existing:
        raise HTTPException(status_code=404, detail="规则不存在")
    await run_db(lambda: update_alarm_rule(rule_id, rule.model_dump()))
...
async def delete_rule(rule_id: int):
    ...
    if not existing:
        raise HTTPException(status_code=404, detail="规则不存在")
    await run_db(lambda: delete_alarm_rule(rule_id))
...
async def list_alarms(status: str = "active"):
    ...
    if status in ("active", "acknowledged", "all"):
        alarms = await run_db(lambda: get_alarms_by_status(status))
    else:
        alarms = await run_db(lambda: get_alarms_by_status("active"))
...
async def ack_alarm(alarm_id: int, user: str = "system"):
    """确认告警"""
    success = await run_db(lambda: acknowledge_alarm(alarm_id, user))
    if success:
